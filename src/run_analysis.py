...
# display form for each protocol code, matching the capitalization used in
# the thesis (DoH, DoQ - not the all-caps DOH/DOQ that .upper() would give)
PROTOCOL_LABELS = {
    "dns": "DNS",
    "doh": "DoH",
    "doq": "DoQ",
...
ORIGIN_LABELS = {
    ...
    # first/third-party, it isn't confirmed non-tracking. so this bucket is
    # a lower bound on real tracker/ad traffic, labelled as such everywhere
...
# small, muted style for panel labels inside multi-panel figures (dashboard,
# or a bar+pie pair showing the same data two ways) - just there to tell
# subplots apart, the real title lives in the LaTeX \caption, not the image
PANEL_LABEL_STYLE = {"fontsize": 12, "color": "#546E7A", "fontweight": "500", "pad": 8}

# figures are sized in inches close to their actual printed width in the
# memoria (see the width= on each \includegraphics), so text at the sizes
# above looks right on the page instead of shrinking ~2x from a screen-sized figure


# thresholds tuned by eyeballing the actual data, not taken from a paper -
# see Chapter 4 of the thesis for how they're used
TOP_N_OUTLIERS = 10
...
def _summarize(rows: list[dict[str, str]], group_key: str, metrics: list[str]):
    ...
    for group, group_rows in sorted(grouped.items()):
        ...
        for metric in metrics:
            ...
            # since a few outliers shouldn't move the "typical" value the
            # way they would with a mean - avg/min/max kept alongside it
...
def _rank_biserial_wilcoxon(values_a: list[float], values_b: list[float]) -> dict[str, Any] | None:
    """paired Wilcoxon test (scipy) plus a rank-biserial effect size, computed
    by hand from the same signed ranks since scipy doesn't give us that part"""
...
def _unique_domains_resolved(profile: dict[str, Any]) -> int:
    """distinct hostnames across a page's resources - basically one DNS lookup
    each. subdomains count separately, since static.files.bbci.co.uk needs
    its own lookup even if www.bbc.com shares the same registered domain"""
    hosts = set()
...
def _dns_overhead_per_resolution(dns_rows: list[dict[str, str]], connection_mode: str) -> float | None:
    ...
    total divided by its repetition count), for the given connection mode.
    Used as one stand-in figure for every domain a page resolves, since the
    handshake/control/payload breakdown already shows the per-resolution
    cost is mostly protocol overhead, not the specific domain being resolved"""
...
def _dns_privacy_cost_rows(
    ...
    """how much of a page's weight comes from choosing an encrypted DNS
    protocol for all the domains it needed, instead of classic DNS - combines
    the DNS-side overhead with the web-side page weight into one number"""
    rows = []
    for connection_mode in ("cold_start", "amortized"):
        ...
        if overhead_per_resolution is None:
            continue
        for row in web_rows:
            # match by filename, not the full path - the CSV keeps whatever
            # path the run directory had at experiment time, which breaks if
            # the run folder gets renamed afterwards (e.g. "..._final")
            profile = profiles_by_file.get(Path(row.get("profile_file", "")).name)
...
def _flag_web_rows(web_rows: list[dict[str, str]]) -> dict[int, str]:
    """flags web visits whose pcap/CDP numbers look like a measurement
    problem rather than real site traffic, so category stats and plots can
    exclude them instead of getting skewed. checked in order, most specific
    reason first: bot-blocked (payload too small, ratio too extreme),
    capture contamination (port-scoping failed, pcap is known unscoped),
    then a plain statistical outlier for whatever's left over
    """
    flags: dict[int, str] = {}
...
def _save_figure(plt, path: Path, dpi: int = 220, pad_inches: float = 0.08) -> None:
    ...
    # outermost glyph (e.g. the last char of a rotated y-axis label) - without
    # it that looks clipped even though nothing's actually missing
...
def _plot_dns_bytes_by_protocol(path: Path, dns_summary: list[dict[str, Any]]) -> bool:
    ...
    what used to be one two-panel figure, so this and the CO2 one below can
    each get their own caption as subfigures in the thesis"""
    if not dns_summary:
        return False
...
def _plot_dns_co2_by_protocol(path: Path, dns_summary: list[dict[str, Any]]) -> bool:
    """median CO2 per resolution, by protocol - same model as the rest of
    the run (Chapter 3). counterpart to the bytes plot above"""
    if not dns_summary:
        return False
...
def _plot_connection_reuse_comparison(path: Path, dns_summary_by_mode: dict[str, list[dict[str, Any]]]) -> bool:
    """cold-start vs amortized overhead-vs-DNS ratio, DoH and DoQ side by
    side. DNS itself is left out - no connection to reuse, so its ratio is
    always 1x either way and would just be a flat line"""
...
def _plot_overhead_breakdown(path: Path, dns_summary: list[dict[str, Any]]) -> bool:
    """handshake/control/payload bytes per protocol, decrypted from the pcap
    with the saved session keys instead of just showing one total (see
    overhead_breakdown.py).

    uses the mean here, not the median like the rest of this file - the mean
    of the three parts always adds up to the mean of the total, but medians
    don't add up that way, so the stacked bar would stop matching the real
    number if we used them
    """
    if not dns_summary:
        return False
    ...
    # same rotated-ylabel clipping issue as the CO2-by-category plot - give
    # it extra padding instead of letting bbox_inches="tight" crop it
...
def _plot_burst_patterns(path: Path, dns_bursts: list[dict[str, Any]], max_domains: int = 3) -> bool:
    ...
    the query is encrypted (DoH/DoQ), the shape of the bursts on the wire
    is still visible and can differ between domains - the fingerprinting
    angle from the thesis.

    each domain has one burst file per repetition, so we only keep the first
    repetition per domain to avoid it showing up twice in the legend
    """
    if not dns_bursts:
        return False
...
def _plot_overhead_by_category(path: Path, web_rows: list[dict[str, str]]) -> bool:
    """overhead % distribution per category - readable no matter how many sites feed each category"""
    if not web_rows:
        return False
    ...
    # used to hide real outlier points in an earlier version of this plot
...
def _plot_cfp_by_category(path: Path, web_category_summary: list[dict[str, Any]]) -> bool:
    ...
    # label above the error bar's cap, not just above the bar - otherwise
    # the text sits inside (or on top of) the error bar when the IQR is
    # wide, which is unreadable
    ...
    # this plot's rotated, mathtext y-axis label ("...kg CO$_2$e)") needs
    # more top padding than the default - bbox_inches="tight" underestimates
    # its extent slightly and crops the last character otherwise
...
def _plot_resource_type_distribution(path: Path, type_summary: list[dict[str, Any]]) -> bool:
    ...
    complements the by-origin view above, this shows what the bytes are,
    not who served them"""
    if not type_summary:
        return False
...
def _markdown_report(
    ...
    if dns_summary_by_mode:
        ...
        # a single-mode run (e.g. old cold_start-only runs) keeps the original,
        # unnested heading levels
...
def analyze_run(run_dir: str | Path):
    ...
    # keyed by filename, not the full path, so a renamed run folder doesn't
    # silently break the match between a CSV row and its profile JSON
    flagged_profile_files = {
        Path(row["profile_file"]).name for row in flagged_web_rows if row.get("profile_file")
    }
    ...
    clean_profiles = [p for p in profiles if Path(p.get("profile_file", "")).name not in flagged_profile_files]
    ...
    profiles_by_file = {Path(p.get("profile_file", "")).name: p for p in profiles}
    ...
    # _summarize more general just for this one case
