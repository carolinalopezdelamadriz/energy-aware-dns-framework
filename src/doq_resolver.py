...
def _make_doq_protocol_class(timeout):
    # imported lazily so this file still loads without aioquic installed -
    # callers get the friendlier RuntimeError below instead
...
async def _resolve_doq_aioquic(domain, host, server_name, port=DEFAULT_DOQ_PORT, timeout=5, keylog_path=None):
    ...
    # append instead of overwrite, so repeated queries in the same run all
    # land in one keylog file that Wireshark can load against the pcap
...
async def _resolve_doq_aioquic_batch(domains, host, server_name, port=DEFAULT_DOQ_PORT, timeout=5, keylog_path=None):
    """Amortized mode: one QUIC connection for the whole batch instead of a
    handshake per query - each domain still gets its own stream"""
    try:
        from aioquic.asyncio.client import connect
        from aioquic.quic.configuration import QuicConfiguration
        protocol_class = _make_doq_protocol_class(timeout)
...
def resolve_doq(domain: str, resolver_name=DEFAULT_DOQ_RESOLVER, use_kdig_fallback: bool = True, keylog_path: str | None = None) -> bool:
    """DoQ lookup (RFC 9250) against the given resolver. Returns a bool, not
    addresses like resolve_classic/resolve_doh - the response is only
    validated, not parsed, and the kdig fallback has no structured response either"""
    ...
    if use_kdig_fallback:
        # kdig has no keylog support, so queries that fall back here
        # won't have their secrets captured
...
def resolve_doq_batch(domains, resolver_name=DEFAULT_DOQ_RESOLVER, keylog_path=None):
    """Amortized version of resolve_doq: one QUIC connection reused for all
    domains. No kdig fallback here - it's a one-shot command, nothing to reuse"""
    resolver = get_doq_resolver(resolver_name)
    try:
        return asyncio.run(
