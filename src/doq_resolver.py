...
async def _resolve_doq_aioquic(
    domain,
    host,
    server_name,
    ...
    try:
        ...
        raise RuntimeError(
            "aioquic is not installed. Run: python3 -m pip install aioquic"
        ) from exc
...
def resolve_doq(domain, resolver_name=DEFAULT_DOQ_RESOLVER, use_kdig_fallback=True):
    """
    Resolve a domain through one selected DoQ resolver.

    The experiment must not silently try several resolvers, because tcpdump would
    capture failed handshakes and retransmissions from all of them. That would
    inflate the measured DoQ byte count.
    """
    ...
    try:
        return asyncio.run(
            _resolve_doq_aioquic(
                domain,
                host=resolver["host"],
                server_name=resolver["server_name"],
            )
        )
    except Exception as exc:
        print(f"DoQ aioquic resolver failed ({resolver['name']}): {exc}")

    if use_kdig_fallback:
        print(f"Trying kdig fallback for DoQ resolver {resolver['name']}...")
