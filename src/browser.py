...
# hostname rather than the full URL. Based on the categories from
# Disconnect's Tracking Protection Lists (same one Firefox uses) - a
# curated subset, not exhaustive, so it favors precision over recall
TRACKER_DOMAINS = (
...
# substrings checked against the full URL, picked to be specific enough
# that they don't collide with unrelated words - a bare "ads" would also
# match inside "uploads" or "downloads"
TRACKER_KEYWORDS = (
    "tracking",
    "tracker",
    "telemetry",
...
# max time to wait for a page to load - not tied to any study, just long
...
def _build_chrome_driver(headless: bool = False, fresh_profile: bool = False, keylog_path: str = None) -> webdriver.Chrome:
    ...
    # TLS/QUIC secrets there, same mechanism as the DoH/DoQ resolvers, so the
    # web pcap can be decrypted in Wireshark too. set it fresh before each
    # Chrome instance so a new visit doesn't inherit a stale path
    if keylog_path:
    ...
    # updates, sync, safe browsing pings...) - same flags Lighthouse uses so
    # a performance measurement isn't skewed by browser housekeeping. a fresh
    # --user-data-dir looks like a first launch, so without these flags every
    # visit re-triggers this traffic - confirmed with a decrypted capture
    # showing real requests to Google's own account/update endpoints
    ...
    # --disable-component-update doesn't cover this one - newer Chrome
    # (150.x here) re-downloads its Optimization Guide model on every fresh
    # profile, confirmed via a decrypted capture where one request to
    ...
    # to serve a bot-challenge page instead of the real content - sites doing
    # TLS or behavioral fingerprinting will still catch a scripted browser
...
class _ChromePortWatcher:
    ...
    interface - otherwise other traffic on the machine ends up mixed in.
    best-effort: if anything fails, no ports get collected and we just
    fall back to an unscoped capture
    """
...
def browse_and_profile(
    ...
    """loads a URL with Selenium + CDP and returns a dict with byte totals by
    resource type and by origin, the raw resource list, and the local ports
    Chrome used (so the pcap can later be scoped to just this visit)
    """
    driver = _build_chrome_driver(headless=headless, fresh_profile=fresh_profile, keylog_path=keylog_path)
    ...
    try:
        ...
        for entry in logs:
            ...
            if method == "Network.responseReceived":
                ...
                # happened with chrome://new-tab-page/* getting logged before the real
                ...
                # cross the network, so they'd never show up in the pcap
                # either. has to be excluded from the pcap vs CDP comparison
                # or the overhead would come out negative
                from_cache = bool(
        ...
        for request_id, size in bytes_by_request.items():
            ...
            if meta is None:
                ...
                # skip it instead of labeling "unknown_origin", which would
                # just reintroduce the bytes we filtered out earlier
                continue
