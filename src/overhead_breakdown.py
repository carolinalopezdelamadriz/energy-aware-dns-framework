...
def _classify_dns(pcap_path, ports=None, base_filter=None):
    # classic DNS is unencrypted with no handshake, so all we need is
    # separating the ethernet+IP+UDP headers from the DNS message itself
...
def _classify_doh(pcap_path, keylog_path=None, ports=None, base_filter=None):
    ...
    for length, stream, syn, tcp_len, fin, rst, content_type in parsed:
        ...
        if syn:
            ...
            # also covers the rare case where content_type couldn't be
            # resolved - in a short cold-start exchange, almost everything
            # left over is real payload anyway
            payload += length
...
def _classify_doq(pcap_path, keylog_path=None, ports=None, base_filter=None):
    ...
    # header (1-RTT) = already-protected DoQ data. A single UDP datagram can
    # coalesce both during the handshake (RFC 9000 §12.2) and there's no way
    # to split them by length, so that whole datagram just counts as
    # handshake - rare, and only happens during the handshake anyway
    #
    # also: within 1-RTT we can't tell an ACK apart from real STREAM data
    # (aioquic's keys aren't enough for tshark to do that), so "payload_bytes"
    # here really just means "bytes after the handshake"
    display_filter = _combine_filters(base_filter, _ports_filter(ports))
...
def breakdown_web_overhead(pcap_path, keylog_path=None, ports=None):
    # same handshake/control/payload split as breakdown_overhead, but for a
    # web visit's pcap, which mixes several connections at once (TCP+TLS and
    # QUIC, both on port 443). that's why each classifier gets restricted to
    # its own transport first - without that filter, a stray QUIC/UDP packet
    # read by the TCP parser would look like tcp.len == 0 and get miscounted
...
def breakdown_overhead(pcap_path, protocol, keylog_path=None):
    # splits a DNS experiment's pcap into handshake/control/payload bytes
    # via tshark, decrypting TLS/QUIC with the keylog if we have one
    if protocol == "dns":
